...
def manifest_to_track_infos(manifest_df: pd.DataFrame) -> list:
    ...
    for _, row in manifest_df.iterrows():
        ...
        for col in manifest_df.columns:
            if col.startswith('gt_') and col.endswith('_path') and col != 'gt_vocal_path':
...
def main():
    parser = argparse.ArgumentParser(description="Create segment index with silence detection")
    parser.add_argument(
        "--config",
        type=str,
    ...
    calibration_path = Path(config.paths.results_root) / "trackA" / "silence_calibration.json"
    ...
    print(f"  Time range: [{segments_df['start_sec'].min():.2f}, {segments_df['end_sec'].max():.2f}]s")
    print(f"  RMS range: [{segments_df['gt_vocal_rms_db'].min():.1f}, {segments_df['gt_vocal_rms_db'].max():.1f}] dB")
