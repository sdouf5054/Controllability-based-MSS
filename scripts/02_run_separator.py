...
def run_separation_batch(
    ...
    mss_root = Path(config.paths.mss_root)
    ...
    print(f"  torchaudio Demucs: {'✓ Available' if torchaudio_available else '✗ Not available'}")
    ...
    elif hasattr(config.separator, 'mss_config_path') and config.separator.mss_config_path:
        config_path = Path(config.separator.mss_config_path)
    
    checkpoint_path = None
    if hasattr(config.separator, 'checkpoint_path') and config.separator.checkpoint_path:
...
def main():
    ...
    if hasattr(config.separator, 'model'):
        print(f"  Model: {config.separator.model}")
