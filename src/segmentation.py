...
def create_segments_for_track(
    track_info: TrackInfo, window_sec: float, hop_sec: float, sr: int = 44100
) -> List[Segment]:
    ...
    n_segments = max(
        0, (track_info.duration_samples - window_samples) // hop_samples + 1
    )
...
def create_segment_index(
    tracks: List[TrackInfo], window_sec: float, hop_sec: float, sr: int = 44100
) -> pd.DataFrame:
    ...
    for track_info in tracks:
        segments = create_segments_for_track(track_info, window_sec, hop_sec, sr)
...
def add_silence_flags(segments_df: pd.DataFrame, threshold_db: float) -> pd.DataFrame:
...
def calibrate_and_flag_silence(
    ...
    - Floor values (truly silent at -100dB) are excluded from percentile calculation
    ...
    # Calibrate threshold (floor values excluded from percentile calculation)
    ...
    # Also flag floor values as silent (truly silent segments)
    floor_mask = rms_values <= floor_db + 0.1
    if floor_mask.any():
        df.loc[floor_mask, "is_silent"] = True
...
def get_track_vocal_paths_from_manifest(manifest_df: pd.DataFrame) -> dict:
...
def print_segment_summary(
    segments_df: pd.DataFrame, calibration: Optional[SilenceCalibrationResult] = None
):
    ...
    if calibration:
        ...
        print(
            f"  RMS range: [{calibration.rms_stats.get('min', 'N/A'):.1f}, "
...
if __name__ == "__main__":
    ...
    segments_df = create_segment_index(tracks, window_sec=3.0, hop_sec=1.5, sr=44100)
    ...
    segments_df, calibration = calibrate_and_flag_silence(
        segments_df, initial_threshold_db=-40.0, method="gap_or_5th_percentile"
    )
