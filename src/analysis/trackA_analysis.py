...
def compute_cue_difficulty_correlation(
    ...
    """
    # Merge on seg_id
    merged = cues_df.merge(difficulty_df[['seg_id', 'sisdr_vocal', 'valid']], on='seg_id')
...
def compute_proxy_difficulty_correlation(
    ...
    if 'valid_proxy' in merged.columns and 'valid_diff' in merged.columns:
        merged = merged[(merged['valid_proxy'] == True) & (merged['valid_diff'] == True)]
    ...
    for proxy_name in proxy_cols:
        ...
        results.append(ProxyAssociation(
...
def compute_cue_contamination(
    cues_est_df: pd.DataFrame,
    cues_gt_df: pd.DataFrame
    ...
    for cue_name in ['r', 's', 'b', 'p']:
        ...
        if cue_name == 'b':
            valid_mask = (
                (merged['beat_available_est'] == True) & 
...
def compute_beat_coverage(
    ...
    if 'track_id' in segments_df.columns:
        ...
        tracks_with_beats = merged[merged['beat_available'] == True]['track_id'].unique()
    ...
    return BeatCoverage(
        ...
        track_coverage_ratio=n_tracks_available / n_tracks_total if n_tracks_total > 0 else 0,
        n_segments_total=n_segments_total,
        n_segments_beat_available=n_segments_beat_available,
        segment_coverage_ratio=n_segments_beat_available / n_segments_total if n_segments_total > 0 else 0
    )
...
def generate_trackA_summary(
    ...
    if len(valid_difficulty) > 0:
        # 명시적 타입 캐스팅으로 np.percentile 경고 해결
        valid_array = np.asarray(valid_difficulty, dtype=float)
        ...
        difficulty_stats = {
            ...
            'p5': float(np.percentile(valid_array, 5)),
            'p25': float(np.percentile(valid_array, 25)),
            'p75': float(np.percentile(valid_array, 75)),
            'p95': float(np.percentile(valid_array, 95))
        }
...
class TrackAAnalyzer:
    ...
    def __init__(
        ...
        self.metadata_root = Path(metadata_root) if metadata_root else self.tables_root.parent / "metadata"
    ...
    def compute_coverage(self):
        ...
        if self.cues_df is not None and self.segments_df is not None:
            self.beat_coverage = compute_beat_coverage(
                self.cues_df, self.segments_df
            )
            print("  Beat coverage computed")
    
    def run_full_analysis(
        self,
        dataset_name: str = "MUSDB18-HQ",
        config_snapshot: Optional[Dict] = None
    ) -> TrackASummary:
        ...
        if silence_path.exists():
            with open(silence_path, 'r') as f:
                ...
                silence_result = SilenceCalibrationResult(
                    ...
                    calibration_method=silence_data.get('calibration_method', 'unknown'),
        ...
        # Generate summary - check for required data
        if self.manifest_df is None:
            raise RuntimeError("Manifest data not loaded. Call load_all_data() first.")
        if self.segments_df is None:
            raise RuntimeError("Segments data not loaded. Call load_all_data() first.")
        if self.difficulty_df is None:
            raise RuntimeError("Difficulty data not loaded. Call load_all_data() first.")
        if self.cues_df is None:
            raise RuntimeError("Cues data not loaded. Call load_all_data() first.")
        if self.proxies_df is None:
            raise RuntimeError("Proxies data not loaded. Call load_all_data() first.")
    ...
    def print_summary(self):
        ...
        print(f"Segments: {self.summary.n_segments_total} total, {self.summary.n_segments_valid} valid")
        ...
        if self.cue_correlations:
            ...
            for cue_name in ['r', 's', 'b', 'p']:
                if cue_name in self.cue_correlations:
                    ...
                    print(f"  {cue_name}(t): ρ = {cc['correlation']:+.3f}{sig} "
        ...
        if self.proxy_associations:
            ...
            for pa in self.proxy_associations:
                ...
                print(f"  {pa.proxy_name}: ρ = {pa.spearman_rho:+.3f}{sig} "
                      f"(p={pa.p_value:.4f}, n={pa.n_samples})")
        
        # Beat coverage
        bc = self.summary.beat_coverage
        print("\n--- Beat Coverage ---")
        print(f"  Tracks: {bc.n_tracks_available}/{bc.n_tracks_total} ({bc.track_coverage_ratio:.1%})")
        print(f"  Segments: {bc.n_segments_beat_available}/{bc.n_segments_total} ({bc.segment_coverage_ratio:.1%})")
...
if __name__ == "__main__":
    ...
    for cue_name, result in correlations.items():
        print(f"  {cue_name}(t): ρ = {result['correlation']:+.3f}, "
