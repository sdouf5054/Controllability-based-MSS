...
def compute_rms_db(audio: np.ndarray, ref: float = 1.0, min_db: float = -100.0) -> float:
...
def get_segment_audio(
    ...
    if end_sample > len(audio):
        raise ValueError(f"end_sample ({end_sample}) exceeds audio length ({len(audio)})")
    if start_sample >= end_sample:
        raise ValueError(f"start_sample ({start_sample}) must be < end_sample ({end_sample})")
...
def calibrate_silence_threshold(
    ...
    # Try to find gap
    gap_threshold = find_histogram_gap(rms_values)
    ...
    if gap_threshold is not None:
        ...
        percentile_threshold = float(np.percentile(rms_values, percentile_cutoff))
