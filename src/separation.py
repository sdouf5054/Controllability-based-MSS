...
def get_cache_path(
    cache_root: Path, model_name: str, track_id: str, stem: str = "vocals"
) -> Path:
...
def run_mss_inference(
    ...
    Returns:
        True if successful
    """
    # --- Normalize to absolute paths for robustness (prevents cwd-relative duplication bugs) ---
    mss_root = Path(mss_root).resolve()
    ...
    inference_script = (mss_root / "inference.py").resolve()
    ...
    # Build command (use absolute path to inference.py; cwd may still be set safely)
    cmd = [
        "micromamba",
        "run",
        "-n",
    ...
    # Add config if specified, else use default config based on model type
    if config_path and config_path.exists():
        cmd.extend(["--config_path", str(config_path)])
    else:
        default_config = (
            mss_root / "configs" / f"config_musdb18_{model_type}.yaml"
        ).resolve()
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            cwd=str(mss_root),  # safe now because all important paths are absolute
        )

        if result.returncode != 0:
            print("MSS inference failed:")
...
def run_torchaudio_demucs(
    input_path: Path, output_dir: Path, device: str = "cuda"
) -> bool:
    ...
    try:
        ...
        for i, stem_name in enumerate(stem_names):
            ...
            torchaudio.save(str(stem_path), sources[i].cpu(), sample_rate)
...
def separate_track(
    ...
    if not success and use_torchaudio_fallback:
        ...
        success = run_torchaudio_demucs(
            input_path=mixture_path, output_dir=track_output_dir, device=device
        )
...
def load_separated_vocal(
    cache_root: Path, model_name: str, track_id: str, sr: int = 44100, mono: bool = True
) -> Optional[Tuple]:
...
def align_separated_with_gt(
    ...
    return align_lengths(
        est_audio, gt_audio, policy=policy, max_diff_samples=max_diff_samples, warn=warn
    )
